...
class DataFrame(_Frame, Generic[T]):
    ...
    def _kser_for(self, label):
        ...
        return Series(
            self._internal.copy(scol=self._internal.spark_column_for(label), column_labels=[label]),
            anchor=self,
        )
...
defaultdict(<class 'list'>, {'col..., 'col...})]
    ...
    def where(self, cond, other=np.nan):
        ...
        return DataFrame(
            kdf._internal.with_new_seires_or_spark_columns(column_scols, column_labels=self._internal.column_labels)
        )
    ...
    def set_index(self, keys, drop=True, append=False, inplace=False):
        ...
        if append:
            ...
            index_map = [(self._internal.spark_column_name_for(label), label) for label in keys]

        internal = self._internal.copy(
            index_map=index_map,
            column_labels=column_labels,
            column_scols=[self._internal.spark_column_for(label) for label in column_labels],
        )
    ...
    def reset_index(self, level=None, drop=False, inplace=False, col_level=0, col_fill=""):
        ...
        new_data_scols = [
            self._internal.spark_column_for(column).alias(name_like_string(name))
    ...
    def _mark_duplicates(self, subset=None, keep="first"):
        if len(self._internal.index_labels) > 1:
        ...
        if self._internal.index_labels[0] is not None:
            name = self._internal.index_labels[0]
        else:
            name = ("0",)
    ...
    def duplicated(self, subset=None, keep="first"):
        ...
        return _col(
            DataFrame(
                _InternalFrame(
                    sdf=sdf,
                    index_map=[(index_column, self._internal.index_labels[0])],
                    column_labels=[name],
    ...
    def _assign(self, kwargs):
        ...
        internal = self._internal.with_new_seires_or_spark_columns(scols, column_labels=column_labels)
        return DataFrame(internal)
    ...
    def pivot(self, index=None, columns=None, values=None):
        ...
        if should_use_existing_index:
            ...
            internal = df._internal.copy(
                index_map=[
                    (index_column, name)
                    for index_column, name in zip(index_columns, self._internal.index_labels)
                ]
            )
            return DataFrame(internal)
    ...
    def columns(self, columns):
        if isinstance(columns, pd.MultiIndex):
            ...
            self._internal = self._internal.with_new_seires_or_spark_columns(
    ...
    def select_dtypes(self, include=None, exclude=None):
        ...
        * To select datetimes, use ``np.datetime64``, ``'datetime'`` or
        ...
        column_scols = [self._internal.spark_column_for(label) for label in column_labels]
        return DataFrame(self._internal.with_new_seires_or_spark_columns(column_scols, column_labels=column_labels))
    ...
    def drop(
        ...
        if labels is not None:
            ...
            column_scols = [self._internal.spark_column_for(label) for label in labels]
            internal = self._internal.with_new_seires_or_spark_columns(column_scols, column_labels=list(labels))
            return DataFrame(internal)
        else:
    ...
    def isin(self, values):
        ...
        return DataFrame(self._internal.with_new_seires_or_spark_columns(column_scols))
    ...
    def merge(
        ...
        if on:
            ...
            if left_index:
                left_keys = self._internal.index_spark_column_names
            else:
                left_keys = _to_list(left_on)
            if right_index:
                right_keys = right._internal.index_spark_column_names
            else:
    ...
    def astype(self, dtype) -> "DataFrame":
        ...
        return DataFrame(self._internal.with_new_seires_or_spark_columns(applied))
    ...
    def _reindex_columns(self, columns):
        ...
        return self._internal.with_new_seires_or_spark_columns(scols, column_labels=labels)
    ...
    def unstack(self):
            ...
            internal = df._internal.copy(
                index_map=[
                    ...
                    for index_column, name in zip(
                        df._internal.index_spark_column_names, self._internal.index_labels[:-1]
                    )
                ],
                column_label_names=(
                    ...
                    + [
                        None
                        if self._internal.index_labels[-1] is None
        ...
        for i, index_name in enumerate(self._internal.index_labels):
    ...
    def rename(
        ...
        if columns_mapper_fn:
            ...
            internal = internal.with_new_seires_or_spark_columns(new_data_scols, column_labels=new_column_labels)
        if inplace:
            self._internal = internal
            return self
        else:
    ...
    def quantile(self, q=0.5, axis=0, numeric_only=True, accuracy=10000):
        ...
        internal = self._internal.copy(
            sdf=sdf,
            column_scols=[scol_for(sdf, col) for col in self._internal.data_spark_column_names],
    ...
    def __array_ufunc__(self, ufunc: Callable, method: str, *inputs: Any, **kwargs: Any):
        ...
        if all(isinstance(inp, DataFrame) for inp in inputs) and any(
            ...
            internal = this._internal.with_new_seires_or_spark_columns(applied)
            return DataFrame(internal)
